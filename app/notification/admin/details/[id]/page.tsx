...
export default function NotificacaoDetalhes() {
  ...
  const [notificacao, setNotificacao] = useState<NotificacaoDetalhe | null>(
    null
  );
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<"detalhes" | "comentarios">(
    "detalhes"
  );
  ...
  const buscarDetalhes = async () => {
    try {
      ...
      setNotificacao(data);
      console.log(data);
      const isUserRead = data.destinatarios.some(
        (dest: Destinatario) => dest.nome === user?.nome && dest.lido == false
      );
      ...
      console.log("asdasmkasdmkasmkdsa", isUserRead);

      const comentariosResponse = await api.get(
        `/Notificacao/${id}/comentarios`
      );
  ...
  const tipoLabel = tiposNotificacao.find(
    (t) => t.value === notificacao.tipo
  )?.label;
  ...
  async function readMessage(notification_id: number) {
    try {
      await api.put(`/Notificacao/${notification_id}/atualizar`, {
        MarcarComoLida: true,
      });
            ...
            <div className="bg-white rounded-lg shadow p-4">
              ...
              <div className="mt-4 text-sm text-gray-700 space-y-1">
                ...
                <p>
                  ...
                  {notificacao.moradorOrigem?.bloco &&
                  notificacao.moradorOrigem?.apartamento
                ...
                <p>
                  ...
                  {formatDateTime(notificacao.ultimaAtualizacao)}
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div
                  className={`transition-all duration-300 overflow-hidden ${
                    showDestinatarios ? "max-h-64" : "max-h-0"
                  }`}
                >
                  <div className="p-2 space-y-1 text-xs">
                    {notificacao.destinatarios.map((d, i) => {
                      console.log("dasasnasnasnjsndjnadnjad", d);
                      return (
                        <div
                          key={i}
                    ...
                    {notificacao.historico.map((h, i) => (
                      <div key={i} className="flex flex-col border-b pb-1">
                        <p>
                          ...
                          {
                            statusOptions.find((s) => s.value === h.statusNovo)
                              ?.label
                          }
                        </p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg w-11/12 max-w-md p-6 relative">
              ...
              <div className="flex flex-col items-start text-lg font-semibold text-gray-400 relative">
                ...
                >
                  ●{" "}
                  <span className={notificacao.status === 1 ? "font-bold" : ""}>
                    Enviada
                  </span>
                </div>
                ...
                {notificacao.status === 3 ? (
                  ...
                  <>
                    ...
                    >
                      ●{" "}
                      <span
                        className={notificacao.status === 2 ? "font-bold" : ""}
                      >
                        Aprovada
                      </span>
                    </div>
                    ...
                    >
                      ●{" "}
                      <span
                        className={notificacao.status === 4 ? "font-bold" : ""}
                      >
                        Em andamento
                      </span>
                    </div>
            ...
            <div className="space-y-2">
              {comentarios.length === 0 ? (
                ...
                comentarios.map((c, index) => (
                  <div
                    key={c.id || index}
                    className="bg-gray-50 border rounded-md p-2 text-sm"
                  >
                    <p>
