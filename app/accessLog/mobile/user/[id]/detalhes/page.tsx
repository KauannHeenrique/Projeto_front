...
export default function DetalhesAcessoPage() {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="sticky top-0 z-20 bg-white border-b px-4 py-2 flex justify-between items-center shadow-sm">
        <Button
          type="button"
          onClick={() =>
          router.push(`/accessLog/mobile/user?apartamentoId=${detalhes?.apartamento}&usuarioId=${id}`)}
          variant="ghost"
  ...
  Documento: {
    ...
    <div className="grid grid-cols-1 md:grid-cols-1 gap-4 space-y-2">
  <div>
    <strong>Nível de Acesso:</strong> {detalhes.nivelAcesso}
  </div>


  <div>
    <strong>Bloco:</strong> {detalhes.bloco} | <strong>Apartamento:</strong> {detalhes.apartamento}
  </div>

  <div>
    <strong>Data:</strong> {formatarData(detalhes.dataHoraEntrada)} | <strong>Hora:</strong> {formatarHora(detalhes.dataHoraEntrada)}
  </div>

  <div>
  <strong>Tipo de Entrada:</strong> {detalhes.entradaPor === "1" ? "TAG" : "Manual"}
  </div>
...
{detalhes.entradaPor === "2" && (
  <div className="pt-6 space-y-2 border-t border-gray-200">
    {detalhes.registradoPor && (
      <div>
        <strong>Registrado por:</strong> {detalhes.registradoPor}
      </div>
    )}
    {detalhes.observacao && (
      <div>
        <strong>Observação:</strong> {detalhes.observacao}
      </div>
    )}
  </div>
)}
