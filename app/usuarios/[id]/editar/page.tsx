...
interface FormData {
  ...
  phone: string;
  document: string;
  accessLevel: "funcionario" | "sindico" | "morador";
  apartmentId: string;
  codigoRFID: string;
}
...
export default function EditUser({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_URL}/api/Usuario/BuscarUsuarioPorId?id=${params.id}`);
        if (!response.ok) {
        ...
        setFormData({
          ...
          document: formatCPF(data.documento || ""),
          accessLevel: data.nivelAcesso === 1 ? "funcionario" : data.nivelAcesso === 3 ? "sindico" : "morador",
          apartmentId: data.apartamentoId?.toString() || "",
          codigoRFID: data.codigoRFID || "",
        });
        setBloco(data.apartamento?.bloco || "");
  ...
  const validateForm = (): FormErrors => {
    ...
    // Ignorar validação de apartamento para funcionario
    if (formData.accessLevel !== "funcionario") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const usuario = {
        ...
        ApartamentoId: formData.accessLevel === "funcionario" ? 0 : parseInt(formData.apartmentId) || 0,
        CodigoRFID: formData.codigoRFID || null,
        Status: true,
      };

      const response = await fetch(`${API_URL}/api/Usuario/AtualizarUsuario`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setApiError(data.mensagem || "Falha ao atualizar usuário");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <h1 className="text-lg sm:text-xl md:text-2xl font-bold mb-6">Editar Morador</h1>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-4 sm:gap-6">
            <div className="flex flex-col w-full sm:w-1/2">
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, accessLevel: e.target.value as "funcionario" | "sindico" | "morador" })}
                disabled={isLoading}
