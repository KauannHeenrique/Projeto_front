...
export default function CriarNotificacaoDesktop() {
  ...
  const buscarApartamentoDestinoId = async (): Promise<number | null> => {
  try {
    const { data } = await api.get(`/Apartamento/BuscarApartamentoPor?bloco=${filtroBloco}&numero=${filtroNumero}`);
    
    if (Array.isArray(data) && data.length > 0) {
      return data[0].id; // ✅ pega o primeiro apartamento encontrado
    }

    return null;
  } catch (error) {
    console.error("Apartamento não encontrado", error);
    return null;
  }
};
  ...
  try {
    ...
    let apartamentoDestinoId: number | null = null;
    ...
    if (tipo === "1") {
      ...
      apartamentoDestinoId = await buscarApartamentoDestinoId();

      if (!apartamentoDestinoId) {
        setMensagemErro("Apartamento informado não encontrado.");
        setLoadingCriar(false);
        return;
      }
    }
    ...
    await api.post("/Notificacao/CriarNotificacao", {
      ...
      apartamentoDestinoId: apartamentoDestinoId, // ✅ agora está correto
    });
