...
export default function RegistroAcessosPage() {
  ...
  const [abaAtiva, setAbaAtiva] = useState<"moradores" | "visitantes">(
    "moradores"
  );
  ...
  const buscarTodosVisitantes = async () => {
    ...
    try {
      const response = await api.get(
        "/AcessoEntradaVisitante/ListarEntradasVisitantes"
      );
  ...
  const handleBuscarVisitantes = async () => {
    ...
    try {
      ...
      if (modoCombinarVisitante) {
        ...
        if (filtroDocumentoVisitante)
          params.append("documento", cleanDocument(filtroDocumentoVisitante));
        ...
        if (dataInicioVisitante.length === 10)
          params.append("dataInicio", formatForBackend(dataInicioVisitante));
        if (dataFimVisitante.length === 10)
          params.append("dataFim", formatForBackend(dataFimVisitante));
      } else {
        if (filtroTipoVisitante === "nome")
          params.append("nome", valorFiltroVisitante);
        if (filtroTipoVisitante === "documento")
          params.append("documento", cleanDocument(valorFiltroVisitante));
  ...
  const handleBuscar = async () => {
    ...
    try {
      ...
      if (modoCombinar) {
        ...
        if (filtroDocumento)
          params.append("documento", cleanDocument(filtroDocumento));
        if (filtroNivel) params.append("nivelAcesso", filtroNivel);
        if (dataInicio.length === 10)
          params.append("dataInicio", formatForBackend(dataInicio));
        if (dataFim.length === 10)
          params.append("dataFim", formatForBackend(dataFim));
      } else {
        if (filtroTipo === "data") {
          if (dataInicio.length === 10)
            params.append("dataInicio", formatForBackend(dataInicio));
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {abaAtiva === "moradores" && (
          <>
            ...
            {erros && (
              <div className="bg-red-100 text-red-700 p-3 rounded">{erros}</div>
            )}
            ...
            {!buscou ? (
              <p className="text-gray-500 mt-6">
                Use os filtros acima para buscar registros de acesso.
              </p>
            ) : acessos.length === 0 ? (
              <p className="text-gray-500 mt-6">
                Nenhum acesso encontrado com os critérios informados.
              </p>
            ) : (
                    ...
                    {acessos.map((a) => (
                      <tr key={a.id} className="border-t">
                        ...
                        <td className="px-4 py-2 text-center">
                          {formatDateOnly(a.dataHoraEntrada)}
                        </td>
                        <td className="px-4 py-2 text-center">
                        ...
                        >
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              router.push(`/accessLog/desktop/${a.id}/detalhes`)
                            }
                          >
                            Ver detalhes
                          </Button>
        ...
        {abaAtiva === "visitantes" && (
          <>
              ...
              <div className="flex gap-2 justify-end">
                <div className="w-full flex flex-col md:flex-row items-end gap-4 justify-between">
                  ...
                  >
                    ...
                    <option value="documento">Documento</option>
                    <option value="data">Data</option>
                    <option value="apartamentoVisitado">
            ...
            {!buscou ? (
              <p className="text-gray-500 mt-6">
                Use os filtros acima para buscar registros de visitantes.
              </p>
            ) : acessos.length === 0 ? (
              <p className="text-gray-500 mt-6">
                Nenhum visitante encontrado com os critérios informados.
              </p>
            ) : (
              <div className="mt-6 overflow-x-auto bg-white rounded shadow">
                <table className="w-full text-sm text-gray-800">
                      ...
                      <tr key={a.id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-right">
                          <Button
                            ...
                            onClick={() =>
                              router.push(`/accessLog/${a.id}/detalhes`)
                            }
                          >
                            Ver detalhes
                          </Button>
                  ...
                  onClick={async () => {
                    try {
                      const response = await api.post(
                        ...
                        {
                          EntradasMorador: true,
                          EntradasVisitante: true,
                        },
                      ...
                      link.setAttribute(
                        "download",
                        "Relatorio_Usuarios_Visitantes.xlsx"
                      );
