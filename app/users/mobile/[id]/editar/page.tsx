...
export default function EditUser() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        setBloco(
          usuario.apartamento?.bloco ? String(usuario.apartamento.bloco) : ""
        );
        setNumero(
          usuario.apartamento?.numero ? String(usuario.apartamento.numero) : ""
        );
      } catch (err: any) {
        const msg =
          err?.response?.data?.mensagem || "Erro ao carregar dados do usuário.";
        setApiError(msg);
      }
    };
  ...
  const buscarApartamento = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.mensagem || "Erro ao buscar apartamento.";
      setApiError(msg);
    }
  };
    ...
    if (formData.accessLevel !== "funcionario") {
      ...
      if (buscaAntes) {
        ...
        if (!formData.apartmentId || formData.apartmentId === "") {
          setIsLoading(false);
          setApiError(
            "Não foi possível localizar o apartamento. Verifique os campos e tente novamente."
          );
          return;
        }
      }
        ...
        <form onSubmit={handleSubmit} id="editUserForm" className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-end">
            ...
            <div className="flex flex-col w-full sm:w-1/2">
              ...
              {errors.codigoRFID && (
                <p className="text-sm text-red-600 mt-1">{errors.codigoRFID}</p>
              )}
            </div>
          </div>
        </form>
